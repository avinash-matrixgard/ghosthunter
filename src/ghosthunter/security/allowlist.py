"""Layer 2: GCP allowlist (v1.0).

The primary security gate. A command must match exactly one of these
patterns. Each pattern uses `\\b` after the action verb so arbitrary
read-only flags (--format, --filter, --project, --limit, ...) can follow.
"""
import re
